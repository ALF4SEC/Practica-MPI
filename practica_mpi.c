...
void proceso_buscador(int id, int num_procs)
{
    ...
    while (!terminado) {
        ...
        if (strstr(mensaje_descifrado, pista) != NULL) {
            ...
            /*
             * La pista NO aparece: comprobar TAG_PARAR cada
             * CHECK_CADA_N_INTENTOS iteraciones para no saturar
             * con llamadas MPI innecesarias, pero sin el coste
             * de una syscall (mygettime/gettimeofday) en cada iteracion.
             */
            if (est.intentos_totales % CHECK_CADA_N_INTENTOS == 0) {
                int flag;
